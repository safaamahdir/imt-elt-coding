...
def create_daily_revenue():
    ...
    SELECT
        ...
        SUM(ol.quantity) AS total_items
    FROM {SILVER_SCHEMA}.fct_orders o
    LEFT JOIN {SILVER_SCHEMA}.fct_order_lines ol
...
def create_product_performance():
    ...
    GROUP BY ol.product_id, ol.product_name, ol.brand, ol.category
...
def create_customer_ltv():
    ...
    SELECT 
        ...
        (MAX(DATE(o.order_date)) - MIN(DATE(o.order_date))) AS days_as_customer
    FROM {SILVER_SCHEMA}.fct_orders o
    JOIN {SILVER_SCHEMA}.dim_users u
        ON o.user_id = u.user_id
    WHERE o.status NOT IN ('cancelled', 'chargeback')
    GROUP BY o.user_id, u.email, u.first_name, u.last_name, u.loyalty_tier
